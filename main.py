...
if __name__ == "__main__":
    ...
    for i, row in osv_tb.iterrows():
        ...
        try:
            if osv_tb.loc[i, 'alpha'] is np.nan:  # void fraction 데이터가 아니라 OSV 실험 데이터에서 xosv 계산
                ...
                print("{}의 {} 실험 데이터의 r2: {}".format(osv_tb.loc[i, 'source'], osv_tb.loc[i, 'run_id'], r2))

                inflectionPoint = round(-linReg.coef_[0, 2] / (3 * linReg.coef_[0, 3]), 4)
                xMartin = linReg.intercept_[0]

                if r2 < 0.9:
                    osv_tb.loc[i, 'xMartin'] = np.nan
                    osv_tb.loc[i, 'serizawaXeq'] = np.nan
                    osv_tb.loc[i, 'martinXeq'] = np.nan
                    osv_tb.loc[i, 'staubXeq'] = np.nan
                else:
                    osv_tb.loc[i, 'xMartin'] = xMartin
                    osv_tb.loc[i, 'serizawaXeq'] = inflectionPoint
                    osv_tb.loc[i, 'martinXeq'] = round(-linReg.intercept_[0] / linReg.coef_[0, 1], 4)
                    osv_tb.loc[i, 'staubXeq'] = round((2 * linReg.coef_[0, 3] * inflectionPoint ** 3 + linReg.coef_[
    ...
    osv_tb['de'] = round(
        osv_tb[['doi', 'dio', 'geo', 'hsur', 'dh']].apply(lambda x: pro.calDe(x[0], x[1], x[2], x[3], x[4]), axis=1), 6)
    osv_tb['pe'] = round(osv_tb[['dh', 'g', 'cpf', 'kf']].apply(lambda x: pro.calPe(x[0], x[1], x[2], x[3]), axis=1), 6)
    osv_tb['st'] = round(
        osv_tb[['q', 'cpf', 'rhof', 'v', 'tosv']].apply(lambda x: pro.calSt(x[0], x[1], x[2], x[3], x[4]), axis=1), 6)
    osv_tb['re'] = round(osv_tb[['g', 'dh', 'muf']].apply(lambda x: pro.calRe(x[0], x[1], x[2]), axis=1), 6)
    osv_tb['we'] = round(
        osv_tb[['rhof', 'v', 'dh', 'sigma']].apply(lambda x: pro.calWe(x[0], x[1], x[2], x[3]), axis=1), 6)
    osv_tb['bd'] = round(
        osv_tb[['rhof', 'rhov', 'dh', 'sigma']].apply(lambda x: pro.calBd(x[0], x[1], x[2], x[3]), axis=1), 6)
    ...
    for i, row in cor_osv_tb.iterrows():  # Apply models or correlations to dataframe
        try:
            # OSV
            cor_osv_tb.loc[i, 'dt_js'], cor_osv_tb.loc[i, 'x_js'] = mod.calJeong(osv_tb.loc[i, 'q'],
                                                                                 osv_tb.loc[i, 'rhof'],
                                                                                 osv_tb.loc[i, 'dh'],
                                                                                 osv_tb.loc[i, 'v'],
                                                                                 osv_tb.loc[i, 'cpf'],
                                                                                 ...
                                                                                 osv_tb.loc[i, 'we'],
                                                                                 osv_tb.loc[i, 'bo'],
                                                                                 osv_tb.loc[i, 'bd'])# Jeong and Shim
            cor_osv_tb.loc[i, 'dt_sz'], cor_osv_tb.loc[i, 'x_sz'] = mod.calSahaZuber(osv_tb.loc[i, 'q'],
                                                                                     ...
                                                                                     osv_tb.loc[i, 'cpf'],
                                                                                     osv_tb.loc[i, 'kf'],
                                                                                     osv_tb.loc[i, 'pe'], osv_tb.loc[
                                                                                         i, 'lam'])  # Saha and Zuber
            cor_osv_tb.loc[i, 'dt_levy'], cor_osv_tb.loc[i, 'x_levy'] = mod.calLevy(osv_tb.loc[i, 'sigma'],
                                                                                    osv_tb.loc[i, 'dh'],
                                                                                    osv_tb.loc[i, 'rhof'],
                                                                                    osv_tb.loc[i, 'muf'],
                                                                                    osv_tb.loc[i, 'kf'],
                                                                                    ...
                                                                                    osv_tb.loc[i, 'lam'],
                                                                                    osv_tb.loc[i, 'v'])  # Levy
            cor_osv_tb.loc[i, 'dt_bowr'], cor_osv_tb.loc[i, 'x_bowr'] = mod.calBowring(osv_tb.loc[i, 'p'],
                                                                                       osv_tb.loc[i, 'q'],
                                                                                       osv_tb.loc[i, 'v'],
                                                                                       osv_tb.loc[i, 'lam'],
                                                                                       osv_tb.loc[i, 'cpf'])  # Bowring
            cor_osv_tb.loc[i, 'dt_unal'], cor_osv_tb.loc[i, 'x_unal'] = mod.calUnal(osv_tb.loc[i, 'q'],
                                                                                    osv_tb.loc[i, 'pr'],
                                                                                    osv_tb.loc[i, 'dh'],
                                                                                    osv_tb.loc[i, 'v'],
                                                                                    osv_tb.loc[i, 'cpf'],
                                                                                    ...
                                                                                    osv_tb.loc[i, 're'],
                                                                                    osv_tb.loc[i, 'refri'],
                                                                                    osv_tb.loc[i, 'lam'])  # Unal
            cor_osv_tb.loc[i, 'dt_msz'], cor_osv_tb.loc[i, 'x_msz'] = mod.calMSZ(osv_tb.loc[i, 'q'],
                                                                                 osv_tb.loc[i, 'rhof'],
                                                                                 osv_tb.loc[i, 'dh'],
                                                                                 osv_tb.loc[i, 'g'],
                                                                                 osv_tb.loc[i, 'cpf'],
                                                                                 ...
                                                                                 osv_tb.loc[i, 'geo'],
                                                                                 osv_tb.loc[i, 'doi'],
                                                                                 osv_tb.loc[i, 'dio'], osv_tb.loc[
                                                                                     i, 'lh'])  # Modified Saha and Zuber (2013)
            cor_osv_tb.loc[i, 'dt_costa'], cor_osv_tb.loc[i, 'x_costa'] = mod.calCosta(osv_tb.loc[i, 'geo'],
                                                                                       osv_tb.loc[i, 'q'],
                                                                                       osv_tb.loc[i, 'v'],
                                                                                       osv_tb.loc[i, 'cpf'],
                                                                                       osv_tb.loc[i, 'lam'])  # Costa
            cor_osv_tb.loc[i, 'dt_griffith'], cor_osv_tb.loc[i, 'x_griffith'] = mod.calGriffith(osv_tb.loc[i, 'q'],
                                                                                                osv_tb.loc[i, 'g'],
                                                                                                osv_tb.loc[i, 'cpf'],
                                                                                                osv_tb.loc[
                                                                                                    i, 'lam'])  # Griffith
            cor_osv_tb.loc[i, 'dt_hancox'], cor_osv_tb.loc[i, 'x_hancox'] = mod.calHancox(osv_tb.loc[i, 'q'],
                                                                                          osv_tb.loc[i, 'cpf'],
                                                                                          osv_tb.loc[i, 'lam'],
                                                                                          osv_tb.loc[i, 'kf'],
                                                                                          osv_tb.loc[i, 'de'],
            ...
            cor_osv_tb.loc[i, 'dt_ha2005'], cor_osv_tb.loc[i, 'x_ha2005'] = mod.calHa2005(osv_tb.loc[i, 'q'],
                                                                                          osv_tb.loc[i, 'dh'],
                                                                                          osv_tb.loc[i, 'kf'],
                                                                                          osv_tb.loc[i, 'cpf'],
                                                                                          osv_tb.loc[i, 'lam'],
                                                                                          osv_tb.loc[
                                                                                              i, 'pe'])  # Ha 2005
            cor_osv_tb.loc[i, 'dt_ha2018'], cor_osv_tb.loc[i, 'x_ha2018'] = mod.calHa2018(osv_tb.loc[i, 'rhof'],
                                                                                          osv_tb.loc[i, 'rhov'],
                                                                                          osv_tb.loc[i, 'lam'],
                                                                                          osv_tb.loc[i, 'cpf'],
                                                                                          osv_tb.loc[i, 'bo'],
                                                                                          osv_tb.loc[i, 'v'])  # Ha 2018
            cor_osv_tb.loc[i, 'dt_dix'], cor_osv_tb.loc[i, 'x_dix'] = mod.calDix(osv_tb.loc[i, 'kf'],
                                                                                 osv_tb.loc[i, 'q'],
                                                                                 osv_tb.loc[i, 'dh'],
                                                                                 osv_tb.loc[i, 'cpf'],
                                                                                 osv_tb.loc[i, 'lam'],
                                                                                 osv_tb.loc[i, 're'],
                                                                                 osv_tb.loc[i, 'pr'])  # Dix (1971)
            cor_osv_tb.loc[i, 'dt_sekoguchi'], cor_osv_tb.loc[i, 'x_sekoguchi'] = mod.calSekoguchi(osv_tb.loc[i, 'q'],
                                                                                                   osv_tb.loc[i, 'g'],
                                                                                                   osv_tb.loc[i, 'cpf'],
                                                                                                   osv_tb.loc[
                                                                                                       i, 'lam'])  # Sekoguchi (1980)
            cor_osv_tb.loc[i, 'dt_psz'], cor_osv_tb.loc[i, 'x_psz'] = mod.calParkSahaZuber(osv_tb.loc[i, 'q'],
                                                                                           osv_tb.loc[i, 'rhof'],
                                                                                           osv_tb.loc[i, 'dh'],
                                                                                           osv_tb.loc[i, 'g'],
                                                                                           osv_tb.loc[i, 'cpf'],
            ...
            cor_osv_tb.loc[i, 'dt_costa'], cor_osv_tb.loc[i, 'x_costa'] = mod.calCosta(osv_tb.loc[i, 'geo'],
                                                                                       ...
                                                                                       osv_tb.loc[i, 'cpf'],
                                                                                       osv_tb.loc[i, 'lam'])
            cor_osv_tb.loc[i, 'dt_kal'], cor_osv_tb.loc[i, 'x_kal'] = mod.calKalitvianski(osv_tb.loc[i, 'q'],
                                                                                          osv_tb.loc[i, 'dh'],
                                                                                          osv_tb.loc[i, 'kf'],
                                                                                          osv_tb.loc[i, 'cpf'],
                                                                                          osv_tb.loc[i, 'lam'],
    ...
    for i in range(0, len(cor_osv_tb)):
        ...
        cor_osv_tb.loc[i, 'tstXeq'] = round(minKey, 4)

        # print(cor_osv_tb.loc[i, 'rmseStaubXeq'], cor_osv_tb.loc[i, 'rmseMartinXeq'], cor_osv_tb.loc[i, 'rmseSerizawaXeq'])

        if cor_osv_tb.loc[i, 'tstXeq'] == abs(round(cor_osv_tb.loc[i, 'rmseMartinXeq'], 4)):
            cor_osv_tb.loc[i, 'sol'] = "Martin"
        elif cor_osv_tb.loc[i, 'tstXeq'] == abs(round(cor_osv_tb.loc[i, 'rmseSerizawaXeq'], 4)):
            cor_osv_tb.loc[i, 'sol'] = "Serizawa"
        elif cor_osv_tb.loc[i, 'tstXeq'] == abs(round(cor_osv_tb.loc[i, 'rmseStaubXeq'], 4)):
    ...
    # Feature analysis
    # 숫자값만 가지는 df 테이블 생성
    df_nur = osv_tb[osv_tb.columns.difference(
