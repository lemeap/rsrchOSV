...
# df
df = dbGetQuery(con, "SELECT T2.we, T2.pe, T2.st, T2.nu, T2.ca, T2.source, T2.geo, T3.g, T3.v, T3.sigma, T3.q, T3.rhof, T3.ec AS real_ec
, T3.bo AS real_bo, T2.refri, T3.pr AS real_pr, T3.st AS real_st, T3.tosv, T3.cpf, T3.dh, T3.de, T3.bd AS real_bd, T3.we AS real_we, T3.re AS real_re, T3.ca AS real_ca, T3.pe AS real_pe, T3.nu AS real_nu
                      FROM rmse_osv_tb T1
                      INNER JOIN dimnsl_param_quan_prop_tb T2
                      ON T1.index = T2.index
                      INNER JOIN prop_osv_tb T3
                      ON T2.index = T3.index
                      WHERE T1.xosv IS NOT NULL
                      AND T3.xosv < 0
                      AND T3.xosv IS NOT NULL
                      --AND T1.rmse_sz > -0.7
...
ggplot(data=df, aes(x=real_pe, y=real_st, col=factor(geo), shape=factor(source))) +
  geom_point() +
...
ggplot(data=df1, aes(y=st, x=bo, col = factor(param_dh), shape = factor(param_ca))) +
  geom_point(size = 2) +
