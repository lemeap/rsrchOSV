...
```{r}
# 회귀분석으로 새로운 모델 만들기
F1=formula(real_ec ~ a*real_we^c)
F2=formula(real_nu ~ a*real_pe^b)
Corl1=nls(F1, start = list(a=0.001, c= 0.1), control = list(maxiter = 50000), data = df)
Corl2=nls(F2, start = list(a=1, b=1), control = list(maxiter = 50000), data = df1)
summary(Corl1)
summary(Corl2)
```
...
ggplot(data=df, aes(x=real_pe, y=real_st, col=factor(geo), shape=factor(geo))) +
  geom_point() +
...
ggplot(data=df1, aes(y=nu, x=pe, col = factor(param_we), shape = factor(param_ca))) +
  geom_point(size = 2) +
